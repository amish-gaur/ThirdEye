...
## Aditya — live querying, phone infra, mobile UX, connection mesh
...
### 4. Webcam ↔ web ↔ mobile connection mesh
The "any device, no extra hardware" promise — make it real.
...
## Rishab — voice agents, concurrent calling, Amazon refund automation
...
- Backed by a call-state machine — no double-dials, no stranded legs, no race conditions with Aditya's inbound handler.
